...
// An Identifier stores information that can be used to distinguish two
// different variables. Each identifier has a type which asserts to check if
// types of two identifiers are same. `IdType` must have a
...
public:
  ...
  /// Check if the two identifiers are equal. Does not assert on the type of
  /// identifiers. Null identifiers are considered not equal.
  bool isEqual(const Identifier &other) const { return value == other.value; }

  bool operator==(const Identifier &other) const { return isEqual(other); }
  bool operator!=(const Identifier &other) const { return !isEqual(other); }

  void print(llvm::raw_ostream &os) const { os << "Id(" << value << ")"; }
...
///
/// PresburgerSpace optionally also supports attaching a Identifier with each
/// non-local variable in space. This is disabled by default. `resetIds` is
/// used to enable/reset these identifiers. These identifiers can be used to
/// check if two variables in two different spaces are actually same variable.
...
public:
  ...
  Identifier &getId(VarKind kind, unsigned pos) {
    assert(VarKind::Local != kind && "Local variables have no identifiers");
...
private:
  unsigned &getVarKind(VarKind kind);
