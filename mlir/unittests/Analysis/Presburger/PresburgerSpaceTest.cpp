...
TEST(PresburgerSpaceTest, insertIdValue) {
  ...
  int values[2] = {0, 1};
  space.atValue(IdKind::Domain, 0) = &values[0];
  space.atValue(IdKind::Domain, 1) = &values[1];
  ...
  EXPECT_EQ(*static_cast<int *>(space.atValue(IdKind::Domain, 2)), values[0]);
  EXPECT_EQ(*static_cast<int *>(space.atValue(IdKind::Domain, 3)), values[1]);
}
...
TEST(PresburgerSpaceTest, removeIdRangeValue) {
  ...
  space.atValue(IdKind::Range, 0) = &values[2];

  // Attach values to symbol identifiers.
  space.atValue(IdKind::Symbol, 0) = &values[3];
  space.atValue(IdKind::Symbol, 1) = &values[4];
  space.atValue(IdKind::Symbol, 2) = &values[5];
  ...
  EXPECT_EQ(*static_cast<int *>(space.atValue(IdKind::Domain, 0)), values[1]);

  // Check if symbol values are attached properly.
  EXPECT_EQ(*static_cast<int *>(space.atValue(IdKind::Range, 0)), values[4]);
  EXPECT_EQ(*static_cast<int *>(space.atValue(IdKind::Range, 1)), values[5]);
}
