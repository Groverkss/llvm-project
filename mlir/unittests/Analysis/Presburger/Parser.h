...
/// Parses a IntegerPolyhedron from a StringRef. It is expected that the string
...
/// are all valid IntegerSet representation and that all of them are in
/// compatible spaces.
...
inline MultiAffineFunction parseMultiAffineFunction(StringRef str) {
  ...
  if (getMultiAffineFunctionFromMap(parseAffineMap(str, &context), multiAff)
          .failed())
    assert(false && "flattening unimplemented for semi-affine integer maps");
  return multiAff;
}

inline PWMAFunction
parsePWMAF(ArrayRef<std::pair<ArrayRef<StringRef>, StringRef>> pieces) {
  assert(!pieces.empty() && "Atleast one piece should be present.");
